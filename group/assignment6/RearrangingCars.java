...
public class RearrangingCars {
	...
	public RearrangingCars(char[] start, char[] end) { 
	...
	private void generateMoves(ArrayList<LinkedList<Move>> allSeqs, char[] configuration, 
		...
		for(int nextCarToMove = 0; nextCarToMove<parkingSize; nextCarToMove++) {
				generateMoves(allSeqs, Arrays.copyOf(configuration, configuration.length), new HashSet<>(previousConfigurations), new LinkedList<>(moves), nextCarToMove);
		}
